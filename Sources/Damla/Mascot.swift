...
/// and waves; done, it smiles with a sparkle; failed, it looks sad; idle, calm; out of date, asleep. Only
/// working and waiting move, and that motion runs in Core Animation (`MascotLoop`), so it costs the app nothing
/// per frame; every other phase is a still drawing.
struct DropletMascot: View {
    ...
    // Pieces of the open-eyed face (working, waiting, idle), shared by the still figure and `MascotLoop`.
    private var ink: Color { Color.black.opacity(0.78) }
    var eyeSize: CGFloat { size * 0.11 }
    /// Vertical offset of the eyes from the drop's centre.
    var eyesCenter: CGFloat { size * 0.16 + (phase == .working ? eyeSize * 0.3 : phase == .waiting ? -eyeSize * 0.25 : 0) }
    func eyes(blink: Bool) -> some View {
        let eye = eyeSize, wide = phase == .waiting ? 1.3 : 1
        return HStack(spacing: size * 0.14) {
            Ellipse().fill(ink).frame(width: eye * wide, height: blink ? eye * 0.15 : eye * 1.25 * wide)
            Ellipse().fill(ink).frame(width: eye * wide, height: blink ? eye * 0.15 : eye * 1.25 * wide)
        }
    }
    @ViewBuilder var mouth: some View {
        let s = size, eyeY = s * 0.16
        if phase == .waiting {
    ...
    /// The drop itself with its glint, no face.
    var drop: some View {
        let s = size
        return ZStack {
            DropShape()
    ...
    var handSymbol: some View {
        Image(systemName: "hand.raised.fill").font(.system(size: size * 0.3, weight: .semibold))
            .foregroundStyle(color).shadow(color: .black.opacity(0.35), radius: size * 0.02)
    }
    var handOffset: CGSize { CGSize(width: size * 0.46, height: -size * 0.12) }
