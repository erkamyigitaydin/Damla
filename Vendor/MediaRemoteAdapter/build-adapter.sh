...
ARCHS=(-arch arm64); [[ "${1:-}" == "--universal" ]] && ARCHS=(-arch arm64 -arch x86_64)
...
PLIST
clang "${ARCHS[@]}" -dynamiclib -fobjc-arc -fvisibility=default -Iinclude -Isrc \
...
clang "${ARCHS[@]}" -fobjc-arc -framework Foundation -framework MediaPlayer -Isrc/test \
