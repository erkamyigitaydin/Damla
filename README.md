...
- **Agent’lar:** Claude Code ve Codex'in desteklenen yaşam döngüsü hook'larından proje adı ve durum gelir. Ayrı sekmede oturumlar listelenir; kapalı çentiğin sağında çalışan/bekleyen/tamamlanan durum görünür. Onay bekleyen oturumlar önceliklidir; tamamlanma simgesi 60 saniye sonra çekilir. `Stop`, yanıtın bittiğini gösterir; işin başarılı veya eksiksiz olduğu anlamına gelmez. 30 dakika yeni sinyal gelmeyen etkin oturum “Durum güncel değil” olur. Normal Claude sohbetleri izlenmez; Claude Code oturumları kapsamdadır.
...
Doğrulama: `--self-test`, klavye zaman aşımı/Esc kurallarını ve agent olay geçişlerini izole dosyalarla kontrol eder. `PYTHONDONTWRITEBYTECODE=1 python3 scripts/test-agent-hooks.py`, kurulumun mevcut ayarları ve hook'ları koruduğunu, yedeklemeyi ve tekrar çalıştırılabilirliğini sınar. Gerçek klavye engelleme/Erişilebilirlik, alıcıya dosya iletimi ve iki agent uygulamasından gerçek hook teslimi ayrıca canlı kontrol gerektirir.
