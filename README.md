...
- **Ayarlar:** Üzerine gelince açılma, girişte başlatma (`SMAppService`; uygulama taşınırsa yeniden açıp kapat), pano kaydı, sistem baloncuğunu gizleme, ekran seçimi (Tümü / Fareyi izle / Çentikli) ve çentiksiz ekran biçimi (Menü çubuğu / Ada).
...
- **Agent’lar:** Claude Code ve Codex'in desteklenen yaşam döngüsü hook'larından proje adı ve durum gelir. Ayrı sekmede oturumlar listelenir. Kapalı çentikte agent'ın **maskotu** görünür: Mac'te kurulu Claude ve ChatGPT/Codex uygulamalarının kendi simgeleri çalışma zamanında alınır (Damla hiçbir marka görseli paketlemez). Çalışırken simge nefes alır ve nane halkayla parlar, sağda üç nokta yanıp söner; onay beklerken kehribar halka ve el rozeti, bitince onay rozeti. Ada dinamiktir: tek etkinlikte simge solda, durum sağda; video/müzik ile agent aynı anda çalışıyorsa yanlar genişler ve her biri kendi yarısını alır (solda kapak + ekolayzer, sağda üç nokta + maskot). Odak sayacı da bir etkinliktir; üçü birden varsa sayaç ve agent gösterilir. Aşama değişince HUD'da da simge ve kısa durum yazısı çıkar; Agent'lar sekmesindeki satırlar aynı maskotu kullanır. Onay bekleyen oturumlar önceliklidir; tamamlanma simgesi 60 saniye sonra çekilir. `Stop`, yanıtın bittiğini gösterir; işin başarılı veya eksiksiz olduğu anlamına gelmez. 30 dakika yeni sinyal gelmeyen etkin oturum “Durum güncel değil” olur. Normal Claude sohbetleri izlenmez; Claude Code oturumları kapsamdadır.
